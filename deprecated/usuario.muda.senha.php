...
/*
    if ( strlen($senha_old)>0 && strlen($senha_new1)>0  ) {
        ...
        if ( $senha_new1!=$senha_new2) {
            $msg="Senha nova diferente da confirmação da senha nova.";
        }else{
            $msg="Trocando a senha...";
            ...
            $usuario = new Usuario();
            $msg = $usuario->changePassword($senha_new1,$senha_old,$id_centro,$id_usuario,$nome);
            Arch::logg("MUDA_SENHA:amountAffectedRows=".$msg->code);
            if ( $msg->code>0 ) {
                $audit = new Auditoria();
                $msg = "Senha trocada corretamente!";
                $audit->report("Altera $id_centro, $id_usuario, $nome");
            }else{
                $msg = "Erro ao trocar de senha. Provavelmente a senha antiga seja incorreta." . $msg->description;
            }
        }
    }
*/
Arch::initView(TRUE);
    ...
    if (Arch::session("nome") != "demo") { 
        echo "<p class=texred><?php echo $msg;?></p>";
